...
static struct gatts_profile_inst ion_profile_tab[PROFILE_NUM] = {
    [PROFILE_APP_IDX] = {
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    switch (event) {
        case ESP_GATTS_REG_EVT:{
            ...
            adv_config_done |= SCAN_RSP_CONFIG_FLAG;
    #endif
            esp_err_t create_attr_ret = esp_ble_gatts_create_attr_tab(gatt_db, gatts_if, ION_IDX_NB, SVC_INST_ID);
            if (create_attr_ret){
        ...
        case ESP_GATTS_EXEC_WRITE_EVT:
            // the length of gattc prepare write data must be less than GATTS_ION_CHAR_VAL_LEN_MAX.
            ESP_LOGI(GATTS_TABLE_TAG, "ESP_GATTS_EXEC_WRITE_EVT");
            example_exec_write_event_env(&prepare_write_env, param);
            break;
        case ESP_GATTS_MTU_EVT:
            ESP_LOGI(GATTS_TABLE_TAG, "ESP_GATTS_MTU_EVT, MTU %d", param->mtu.mtu);
            if (param->mtu.mtu <= GATTS_ION_CHAR_VAL_LEN_MAX) {
        ...
        case ESP_GATTS_CREAT_ATTR_TAB_EVT:{
            ...
            else if (param->add_attr_tab.num_handle != ION_IDX_NB){
                ESP_LOGE(GATTS_TABLE_TAG, "create attribute table abnormally, num_handle (%d) \
                        doesn't equal to ION_IDX_NB(%d)", param->add_attr_tab.num_handle, ION_IDX_NB);
            }
            else {
                ESP_LOGI(GATTS_TABLE_TAG, "create attribute table successfully, the number handle = %d\n",param->add_attr_tab.num_handle);
                memcpy(ion_handle_table, param->add_attr_tab.handles, sizeof(ion_handle_table));
                esp_ble_gatts_start_service(ion_handle_table[IDX_SVC]);
            }
            break;
        }
...
static void gatts_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    if (event == ESP_GATTS_REG_EVT) {
        if (param->reg.status == ESP_GATT_OK) {
            ion_profile_tab[PROFILE_APP_IDX].gatts_if = gatts_if;
        } else {
    ...
    do {
        ...
        for (idx = 0; idx < PROFILE_NUM; idx++) {
            ...
            if (gatts_if == ESP_GATT_IF_NONE || gatts_if == ion_profile_tab[idx].gatts_if) {
                if (ion_profile_tab[idx].gatts_cb) {
                    ion_profile_tab[idx].gatts_cb(event, gatts_if, param);
                }
            }
        }
    } while (0);
...
void tm_ble_gatts_server_init(void)
{
    ...
    ret = esp_ble_gatts_app_register(ION_APP_ID);
    if (ret){

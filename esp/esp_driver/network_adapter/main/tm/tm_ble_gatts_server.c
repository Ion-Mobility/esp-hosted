...
#define BATTERY_PROFILE_NUM                         1
#define BATTERY_PROFILE_APP_IDX                     0
#define ESP_BATTERY_APP_ID                          0x55
...
static uint16_t profile_handle_table[BAT_IDX_NB];

EventGroupHandle_t ion_ble_event_group;
static uint8_t ion_ext_adv_raw_data[] = {
        // {len, type, value}
        //flags
        0x02, 0x01, 0x06,
        //txpower
...
static struct gatts_profile_inst battery_profile_tab[BATTERY_PROFILE_NUM] = {
...
static const uint16_t GATTS_CHAR_UUID_TEST_A       = 0x2a19;
...
static const uint8_t battery_level_ccc[2]          = {0x00, 0x00};
static const uint8_t battery_value[1]              = {0x34};
#define CHAR_DECLARATION_SIZE (sizeof(uint8_t))
#define SVC_INST_ID 0


/* Full Database Description - Used to add attributes into the database */
static const esp_gatts_attr_db_t gatt_db[BAT_IDX_NB] =
{
    // Service Declaration
    [BAT_IDX_SVC]        =
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&GATTS_CHAR_UUID_TEST_A, ESP_GATT_PERM_READ,
      GATTS_CHAR_VAL_LEN_MAX, sizeof(battery_value), (uint8_t *)battery_value}},

    /* Client Characteristic Configuration Descriptor */
    [BAT_IDX_CHAR_CFG_A]  =
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&character_client_config_uuid, ESP_GATT_PERM_READ,
      sizeof(uint16_t), sizeof(battery_level_ccc), (uint8_t *)battery_level_ccc}},
    // /* Characteristic Declaration */
    // [IDX_CHAR_B]      =
    // {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&character_declaration_uuid, ESP_GATT_PERM_READ,
    //   CHAR_DECLARATION_SIZE, CHAR_DECLARATION_SIZE, (uint8_t *)&char_prop_read}},
...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_EXT_ADV_SET_PARAMS_COMPLETE_EVT:
        ...
        esp_ble_gap_config_ext_adv_data_raw(EXT_ADV_HANDLE,  sizeof(ion_ext_adv_raw_data), &ion_ext_adv_raw_data[0]);
        break;
    ...
    case ESP_GAP_BLE_PASSKEY_REQ_EVT:                           /* passkey request event */
        ...
        //esp_ble_passkey_reply(battery_profile_tab[BATTERY_PROFILE_APP_IDX].remote_bda, true, 0x00);
        break;
    ...
    case ESP_GAP_BLE_NC_REQ_EVT:
        //vle: get here to generate passkey
        ...
        xEventGroupSetBits(ion_ble_event_group, REQUEST_TO_PAIR);
        break;
    ...
    case ESP_GAP_BLE_KEY_EVT:
        // vle: get here when waiting for user to confirm the passkeys are the same
        //shows the ble key info share with peer device to the user.
        ESP_LOGI(GATTS_TABLE_TAG, "key type = %s", esp_key_type_to_str(param->ble_security.ble_key.key_type));
        xEventGroupSetBits(ion_ble_event_group, CONFIRM_PASSKEY);
        break;
    case ESP_GAP_BLE_AUTH_CMPL_EVT: {
        // vle: get here if the device is already paired before
    ...
    case ESP_GAP_BLE_UPDATE_CONN_PARAMS_EVT:
        // vle: get here if this is a new device and request to pair
                                        ...
                                        esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    switch (event) {
        case ESP_GATTS_REG_EVT:
            ...
            esp_ble_gatts_create_attr_tab(gatt_db, gatts_if, BAT_IDX_NB, SVC_INST_ID);
            break;
        ...
        case ESP_GATTS_CONNECT_EVT:
            //vle: get here when user press connect
        ...
        case ESP_GATTS_CREAT_ATTR_TAB_EVT: {
            ...
            if (param->create.status == ESP_GATT_OK){
                if(param->add_attr_tab.num_handle == BAT_IDX_NB) {
                    memcpy(profile_handle_table, param->add_attr_tab.handles,
                    sizeof(profile_handle_table));
                   esp_ble_gatts_start_service(profile_handle_table[BAT_IDX_SVC]);
                }else{
                    ESP_LOGE(GATTS_TABLE_TAG, "Create attribute table abnormally, num_handle (%d) doesn't equal to HRS_BAT_IDX_NB(%d)",
                         param->add_attr_tab.num_handle, BAT_IDX_NB);
                }
            }else{
                                ...
                                esp_ble_gatts_cb_param_t *param)
{
    ...
    if (event == ESP_GATTS_REG_EVT) {
        if (param->reg.status == ESP_GATT_OK) {
            battery_profile_tab[BATTERY_PROFILE_APP_IDX].gatts_if = gatts_if;
        } else {
    ...
    do {
        int idx;
        for (idx = 0; idx < BATTERY_PROFILE_NUM; idx++) {
            if (gatts_if == ESP_GATT_IF_NONE || /* ESP_GATT_IF_NONE, not specify a certain gatt_if, need to call every profile cb function */
                    gatts_if == battery_profile_tab[idx].gatts_if) {
                if (battery_profile_tab[idx].gatts_cb) {
                    battery_profile_tab[idx].gatts_cb(event, gatts_if, param);
                }
            }
        }
    } while (0);
...
void tm_ble_gatts_server_init(void)
{
    esp_err_t ret;
